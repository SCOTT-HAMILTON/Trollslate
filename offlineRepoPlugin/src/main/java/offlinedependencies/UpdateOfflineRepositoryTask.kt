...
open class UpdateOfflineRepositoryTask : DefaultTask() {
    ...
    private fun withRepositoryFiles(callback: (Map<ModuleComponentIdentifier, Set<File>>) -> Unit) {
        ...
        val extension =
            project.extensions.getByName(OfflineDependenciesPlugin.EXTENSION_NAME) as
                OfflineDependenciesExtension
    ...
    private fun getConfigurations(): Set<Configuration> = project.configurations.toSet()
    private fun collectRepoFiles(
            ...
            configurations
                .map { config ->
                    config
                        .allDependencies
                        .map { dep ->
                            getImperativeDepsArtifacts().map { it.id to it.files } +
                                dep.artifactToComponents().map { it.id to it.files }
                        }
                        .flatten()
                }
                .flatten()
    ...
    private fun collectPoms(components: MutableMap<ModuleComponentIdentifier, MutableSet<File>>) {
        val imperativePomComponents =
            listOf(
                //            SimpleComponentId(
                //                "org.gradle.kotlin.kotlin-dsl",
                //                "org.gradle.kotlin.kotlin-dsl.gradle.plugin",
                // "2.1.7").toModuleCompId(),
                //            SimpleComponentId(
        ...
        components.keys.forEach { logger.info("Component POM ID:$it") }
        val mavenArtifacts =
            project
                .dependencies
                .createArtifactResolutionQuery()
                .forComponents(components.keys + imperativePomComponents)
            ...
            if (poms.isNotEmpty()) {
                when (val pomArtifact = poms.first()) {
                    ...
                    is ResolvedArtifactResult -> {
                        ...
                        logger.info(
                            "Adding pom for component'{}' (location '{}')",
                            component.id,
                            pomFile
                        )
    ...
    private fun collectIvyXmls(
        components: MutableMap<ModuleComponentIdentifier, MutableSet<File>>
    ) {
    ...
    private fun moduleDirectory(ci: ModuleComponentIdentifier): File =
        File(offlineRepoDir, "${ci.group.replace('.', '/')}/${ci.module}/${ci.version}")

    private fun getImperativeDepsArtifacts(): Set<ArtifactComponent> {
        val imperativeDepsList =
            listOf(
                // internals
    ...
    private fun <A> MutableMap<
        ModuleComponentIdentifier, MutableSet<File>>.addFromComponentArtifacts(
        ...
        sources.forEach {
            if (it is ResolvedArtifactResult) {
                logger.trace(
                    "Adding sources for component'${component.id}' (location '${it.file}')"
                )
    ...
    private fun Configuration.resolveToAllRecurseArtifacts(): Set<ArtifactComponent> =
        resolvedConfiguration
            .resolvedArtifacts
            .map { artifact -> artifact.toArtifactComponent() }
            .toSet() +
            resolvedConfiguration
                .firstLevelModuleDependencies
                .map {
                    it.allModuleArtifacts.map { artifact -> artifact.toArtifactComponent() }.toSet()
                }
                .flatten()
                .toSet()
}
