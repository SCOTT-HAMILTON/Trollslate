...
fun Letter(
    ...
    if (codon == null) {
        ...
        Box (modifier = modifier.fillMaxSize()) {
            Codon2Letter(modifier.align(Alignment.Center), canvas_size, codon)
        }
    }
}
...
private fun Codon2Letter(
    ...
    val indivRanges = groupRanges.foldRight(
        GroupRange(0, 0) to mutableListOf<TypedRange>()) { t, r ->
        println("t=$t, r=$r")
        val prev = r.first
        if (prev.range != 0..0) {
        ...
        .sortedBy { it.range.first }.let {
            ...
            if (first.type == RangeType.INDIVIDUALS) {
                ...
                if (start_pos == null) {
                    ...
                    listOf(IndivRange(first.range.first + 1, first.range.last)) + it.drop(1) to start_pos
                }
            } else {
...
//    println("lol split output: indivRanges = $indivRanges, groupRanges: $groupRanges, sortedRanges=$sortedRanges")
    Canvas(
        modifier = modifier.size(canvas_size.width.dp, canvas_size.height.dp),
        onDraw = {
            val path = Path()
...
fun pcToPoint(pcPoint: Point, canvas_size: SizeF) =
    PointF(canvas_size.width*pcPoint.x.toFloat()/100f,
