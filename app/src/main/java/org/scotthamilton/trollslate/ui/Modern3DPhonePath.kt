...
fun Modern3DPhonePath(phoneDims: SizeF) : Path {
    ...
    return Path().apply {
        val radius = CornerRadius(phoneDims.width*0.05f, phoneDims.width*0.05f)
        addRoundRect(
            RoundRect(
                0f, 0f, phoneDims.width, phoneDims.height,
        ...
        FontData.lettersCodonTable['A']?.let { codon ->
            ...
            Codon2LetterPath(codon, size)?.let {
                val rotatedPath = it.asAndroidPath()?.apply {
                    transform(
                        Matrix().also { matrix ->
...
fun Canvas3DPhone(
    ...
    Canvas(
        ...
        onDraw = {
            ...
            val phoneDims = if (ratio > 2.0f) {
                SizeF(size.height*2f, size.height)
            } else {
                SizeF(size.width, size.width/2f)
            }
            val phonePath = Modern3DPhonePath(phoneDims)
            val lines = phonePath.asAndroidPath().approximate(0.5f)
                .asIterable().chunked(3).map {  PointF(it[1], it[2]) }
            println("lines=$lines")
...
//                    style = Stroke(width = size.width*0.007f))
            }
        }
    )
}
