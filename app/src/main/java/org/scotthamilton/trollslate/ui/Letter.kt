...
fun Codon2LetterPath(codon: String, canvas_size: Size): Path? {
    val croppedCodon = codon
    val groupRanges =
        listOf(GroupRange(-1, -1)) +
            """\(\w*\)"""
                .toRegex()
                .findAll(croppedCodon)
    ...
    val indivRanges =
        groupRanges
            ...
            .second
    val (sortedRanges, firstPos) =
        (indivRanges + groupRanges.dropLast(1).drop(1)).sortedBy { it.range.first }.let {
            val first = it.first()
    ...
    sortedRanges.forEach {
        if (it.type == RangeType.INDIVIDUALS) {
            cur_pos_pc =
                applyIndivCode2Path(croppedCodon.substring(it.range), path, cur_pos_pc, canvas_size)
        } else {
            cur_pos_pc =
                applyGroupCode2Path(croppedCodon.substring(it.range), path, cur_pos_pc, canvas_size)
        }
    }
    return path
}
...
private fun Codon2Letter(
    ...
    //    println("lol split output: indivRanges = $indivRanges, groupRanges: $groupRanges,
...
fun pcToPoint(pcPoint: Point, canvas_size: Size) =
    PointF(
        canvas_size.width.toFloat() * pcPoint.x.toFloat() / 100f,
...
fun Letter(
    ...
    if (codon == null) {
        ...
        Box(modifier = modifier.fillMaxSize().background(backgroundColor).padding(start = 10.dp)) {
            Codon2Letter(
