    ...
    sortedRanges.forEach {
        curPosPc =
            if (it.type == RangeType.INDIVIDUALS) {
                applyIndivCode2Path(codon.substring(it.range), path, curPosPc, canvas_size)
            } else {
                applyGroupCode2Path(codon.substring(it.range), path, curPosPc, canvas_size)
            }
    }
    return path
}
...
@RequiresApi(Build.VERSION_CODES.N)
@Composable
fun Letter(modifier: Modifier, letter: Char = 'A', backgroundColor: Color, textColor: Color) {
